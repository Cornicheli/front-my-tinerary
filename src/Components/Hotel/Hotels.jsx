    ...
    return hotelList.map((e) => {
      return (
        <div className="card-hotel">
          <img
            key={e.id}
            className="img-card-hotel"
            src={getRandomImage(e.photo)}
            alt="hotel"
          />
          <h3 className="text-hotel-card" key={e.id}>
            {e.name}
          </h3>
          <Link
            className="text-hotel-card"
            key={e.id}
            to={`/DetailsHotel/${e._id}`}
          >
            See More
          </Link>
        </div>
  ...
  return (
    <>
      <div className="main-hotel">
        ...
        <div className="hotels-container">{<DataFetching />}</div>
      </div>
    </>
  );
}
